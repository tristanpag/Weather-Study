...
double Pre2Alt_ISA(double press) {
    double alt[24];
    double pressure[24];
    ...
    //printf("press: %f  pressure[0]: %f\n", press, pressure[0]);
    /**  interpolate the value of the pressure altitude with the inputed pressure **/
    double tempo = 0;
    //printf("press: %f\n", press);
    if (press > pressure[0] || press < pressure[23]) {
        //printf("Pressure out of range of interpolation\n");
    } else {
        int i = 0;
        while (press < pressure[i]) i++;
        double lambda;
        lambda = (pressure[i-1] - press) / (pressure[i-1] - pressure[i]);
        //printf("i: %d\n", i);
        //printf("lambda: %f\n", lambda);
        tempo = alt[i] + lambda * (alt[i] - alt[i-1]);
    }
    return tempo;
}
